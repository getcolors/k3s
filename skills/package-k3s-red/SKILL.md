---
name: package-k3s-red
description: Creates and operates a secure single-node K3s server using the shared compute library with Red, OpenTofu, Ansible, Flux CD, and a public Git deployment repository. Use when initializing a K3s project, writing colors.yml, building or dry-running infrastructure, provisioning or destroying the server, configuring Flux, or invoking kubectl.
license: MIT
---
...
- Never open TCP 6443 publicly. `./red kubectl` uses SSH.

## Requirements

Bun runs the launcher. Lifecycle commands need OpenTofu and Ansible;
...
./red kubectl apply -f - < manifest.yml
./red delete
```

## Initialize in the current directory

1. Copy the `red` payload beside this file into the project root and make it
   executable.
...
`./red kubectl` runs the server's `sudo k3s kubectl` through the managed SSH
