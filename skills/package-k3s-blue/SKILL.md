---
name: package-k3s-blue
description: Creates and operates a secure single-node K3s server using the shared compute library with Blue, OpenTofu, Ansible, Flux CD, and a public Git deployment repository. Use when initializing a K3s project, writing colors.yml, building or dry-running infrastructure, provisioning or destroying the server, configuring Flux, or invoking kubectl.
license: MIT
---
...
- Never open TCP 6443 publicly. `./blue kubectl` uses SSH.

## Requirements

uv runs the launcher. Lifecycle commands need OpenTofu and Ansible;
...
./blue kubectl apply -f - < manifest.yml
./blue delete
```

## Initialize in the current directory

1. Copy the `blue` payload beside this file into the project root and make it
   executable.
...
`./blue kubectl` runs the server's `sudo k3s kubectl` through the managed SSH
