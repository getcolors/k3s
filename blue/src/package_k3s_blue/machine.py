...
def requirements(opts):
    ingress = [{'id':'icmp','protocol':'icmp','from_port':None,'to_port':None,'sources':['0.0.0.0/0']}]
    ...
    return {'single_host': True, 'private': False, 'security': {'ingress': ingress, 'egress': 'all', 'private_filter': False},
            'legacy_state_keys': [str(opts.get('profile')) + '/k3s-compute.tfstate']}
...
async def step(opts):
    ...
    if planning:
        directory = Path(stage_dir(opts, 'k3s-compute'))
    ...
    return {**opts, **adopted, 'k3s/compute-params': adopted, 'colors-compute/cluster': result['cluster'], 'blue/exit': 0}
