(ns io.github.getcolors.k3s.machine
...
(defn requirements [opts]
  {:single_host true :private false
   :security {:egress "all" :private_filter false
              :ingress (into [{:id "icmp" :protocol "icmp" :from_port nil :to_port nil :sources ["0.0.0.0/0"]}] (mapv (fn [[id port]]
   ...
   :legacy_state_keys [(str (:profile opts) "/k3s-compute.tfstate")]})
(defn errors [opts]
  ...
  (let [planning? (or (= :build (:green/event opts)) (:green/dry-run opts))
    ...
    (when planning?
      (doseq [[stage documents] (cons ["shared" (get-in result [:documents :shared])]
        ...
        (let [target (io/file (cli/stage-dir opts "k3s-compute") stage filename)]
