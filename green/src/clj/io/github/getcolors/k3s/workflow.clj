  ...
  ([opts env]
   (lifecycle/preflight
    opts {:defaults defaults :overlay green-cli/read-pars
          ...
          :after-validate (fn [o e c] (if (and (:real? c) (= :delete (:event c))) (machine/load-inventory o e) (assoc o :green/exit 0)))}
    env)))

(defn ansible-cleanup-step
  "Remove the SSH block and both rendered Ansible trees before compute destroy."
  [opts]
  (let [result (tools/ansible-local-step opts)] (if (wf/failed? result) result (tools/ansible-remote-step result))))
