...
launcher="$root/skills/package-k3s-green/k3s"
tmp=$(mktemp -d)
trap 'rm -rf "$tmp"' EXIT
...
if echo "$pin" | grep -q 'nil'; then
  ...
  out=$( (cd "$copy" && ./k3s build 2>&1) || true )
...
out=$( (cd "$copy" && K3S_LIB_ROOT="$root" ./k3s build 2>&1) ) ||
...
#
# A user runs k3s from wherever they happen to be in their project, not from
# its root. Only the launcher does this walk; nothing in the library can.

mkdir -p "$copy/deep/nested"
out=$( (cd "$copy/deep/nested" && K3S_LIB_ROOT="$root" ./../../k3s build 2>&1) ) ||
...
out=$( (cd "$copy" && K3S_LIB_ROOT="$root" ./k3s frobnicate 2>&1) || true )
