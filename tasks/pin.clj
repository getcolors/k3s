(ns pin
  ...
  K3s's launcher pins only k3s. `once` and `green` come transitively from
  k3s's deps.edn, so there is one site here rather than ONCE's three."
  (:require
...
(def ^:private site
  "Where a copied payload records which k3s commit to resolve.
  ...
  {:path "skills/package-k3s-green/green"
   :rx #"\(def \^:private k3s-sha (nil|\"[0-9a-f]{40}\")\)"})
  ...
  (let [file (io/file (:path site))]
    (cond
      ...
      (let [text (slurp file)
        ...
        (if-not current
          {:green/exit 2
           :green/err (str "could not locate k3s-sha in " (:path site))}
          (let [[head err] (repo-head "." "k3s")]
            (cond
