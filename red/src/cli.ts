...
import * as kubectl from "./kubectl.ts";
import { k3sWorkflow } from "./workflow.ts";
...
export async function run(...args: string[]): Promise<Opts> {
  ...
  if (lifecycleCommands.includes(command)) {
    return runCli(k3sWorkflow, defaultArgs(args));
  }
  if (command === "kubectl") {
    return kubectl.run(defaultFile(), args.slice(1));
  }
...
export async function exec(args: string[] = Bun.argv.slice(2)): Promise<never> {
  if (lifecycleCommands.includes(args[0] ?? "")) {
    return execCli(k3sWorkflow, defaultArgs(args));
  }
