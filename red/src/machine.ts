/** K3s's single host requirement and application parameter adapter. */
...
export function requirements(opts: Opts) {
  ...
  ingress.unshift({id:'icmp',protocol:'icmp',from_port:null as any,to_port:null as any,sources:['0.0.0.0/0']});
  return {single_host:true,private:false,security:{ingress,egress:'all',private_filter:false},legacy_state_keys:[String(opts.profile)+'/k3s-compute.tfstate']};
}
...
export async function step(opts: Opts): Promise<Opts> {
  ...
  if(planning)for(const [stage,documents] of [['shared',result.documents.shared],...Object.entries(result.documents.nodes).map(([id,docs])=>['nodes/'+id,docs])] as [string,Record<string,unknown>][]){
    ...
    const target=join(stageDir(opts,'k3s-compute'),stage,filename);mkdirSync(dirname(target),{recursive:true});writeFileSync(target,JSON.stringify(sorted(document),null,2)+'\n');
  }}
  if(!result.cluster)return {...opts,'red/exit':0};
  const adopted=params(opts,result);return {...opts,...adopted,'k3s/compute-params':adopted,'colors-compute/cluster':result.cluster,'red/exit':0};
}
